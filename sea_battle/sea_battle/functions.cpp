...
int shoot_player(field& _field_cpu, field& _field_player_empty)
{
	...
	console_cursor(0, 32);
	std::cout << convert_ANSI_string_to_ASCII_string("ââåäèòå êîîðäèíàòó ïî îñè x: ");
	std::cin >> x;
	X = columns_indexes[static_cast<int>(x) - 65];
	std::cout << convert_ANSI_string_to_ASCII_string("ââåäèòå êîîðäèíàòó ïî îñè y: ");
	std::cin >> Y;
	Y = rows_indexes[Y - 1];
...
bool shoot_cpu(field& _field_player, field& _field_cpu_empty)
{
	...
	int length;
	   // ################################################## //
	  //********* èíèöèàëèçàöèÿ äâóìåðíîãî ìàññèâà *********//
	 //********* êîîðäèíàòàìè îòðåñóìîãî ïîëÿ *************//
	// ################################################## //
	if (Initialization)
	{
		for (size_t i = 0; i < size; ++i)
...
void draw_border(const int& x, const int& y)
{
	...
	std::cout << convert_ANSI_string_to_ASCII_string("öâåò ãðàíèöû\n");
	color_selection(color_border);
	std::cout << convert_ANSI_string_to_ASCII_string("öâåò ôîíà ãðàíèöû\n");
